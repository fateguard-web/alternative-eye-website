interface BrandButtonProps {
  variant: 'youtube' | 'spotify';
  href: string;
  children: React.ReactNode;
}

export function BrandButton({ variant, href, children }: BrandButtonProps) {
  const variantStyles = {
    youtube: 'bg-[#ff0000] hover:bg-[#e60000] hover:shadow-[0_5px_20px_rgba(255,0,0,0.4)]',
    spotify: 'bg-[#1DB954] hover:bg-[#1ed760] hover:shadow-[0_5px_20px_rgba(29,185,84,0.4)] text-black',
  };
  ...
  return (
    <a
      ...
      className={`flex items-center gap-3 px-8 py-4 font-heading text-xl text-white no-underline rounded-full transition-all duration-200 border-none cursor-pointer hover:transform hover:-translate-y-1 ${variantStyles[variant]}`}
    >
      {icons[variant]}
      {children}
    </a>
